...
void lv_draw_sw_init(void)
{
    ...
    uint32_t sw_draw_cnt = 1;       //保持一个核任务,避免多任务切换带来调度产生的消耗!
#else
    uint32_t sw_draw_cnt = LV_DRAW_SW_DRAW_UNIT_CNT;
#endif
    uint32_t i;
    for(i = 0; i < sw_draw_cnt; i++) {
...
#if LV_USE_OS
        lv_thread_init(&draw_sw_unit->thread, LV_DRAW_THREAD_PRIO, render_thread_cb, LV_DRAW_THREAD_STACK_SIZE, draw_sw_unit);
#endif
    }
