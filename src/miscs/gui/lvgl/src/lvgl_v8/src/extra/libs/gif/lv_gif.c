...
static void lv_register_gif(lv_obj_t * gif_obj) {
    ...
    gif_node_t * new_node = lv_mem_alloc(sizeof(gif_node_t));
...
void lv_gif_pause(lv_obj_t * gif_obj) {
    if(!gif_obj) return;
    
    lv_gif_t * gif = (lv_gif_t *)gif_obj;
    gif_node_t * curr = lv_gif_get_node(gif_obj);
...
void lv_gif_resume(lv_obj_t * gif_obj) {
    if(!gif_obj) return;
