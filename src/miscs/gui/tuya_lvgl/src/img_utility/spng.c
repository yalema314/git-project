...
#define SPNG_MEMORY_ALIGNED             //内存对齐
#ifdef SPNG_MEMORY_ALIGNED
#define SPNG_MEMORY_ALIGNED_SIZE        32
#endif
        ...
        #define SPNG_DISABLE_OPT
        #endif
        #ifdef SPNG_ARM             //以下涂鸦私有定义宏
        #define SIMD_USE_CMSIS_DSP                          //cortex-m 支持的dsp替换库
        //#define SIMD_USE_CMSIS_DSP_DEFILTER_ACCELERATE      //支持复杂带滤波图形解码加速(在tuyaos/platform.mk开启:-mcpu=cortex-m33):cmsis-dsp指令处理暂时有问题?
        #endif
    #ifndef SPNG_DISABLE_OPT
        #ifdef SIMD_USE_CMSIS_DSP_DEFILTER_ACCELERATE
...
#ifndef SPNG_DISABLE_OPT
#ifdef SIMD_USE_CMSIS_DSP_DEFILTER_ACCELERATE           //@2025/2/20,Mikeliu:24位深png及32位深带滤波功能的图片会花图,CMSIS-DSP接口的优化带改善!
...
#ifdef SPNG_MEMORY_ALIGNED
static void *spng_aligned_malloc(size_t alignment, size_t size)
{
    void *original_ptr = tkl_system_psram_malloc(size + alignment - 1 + sizeof(void*));
    if (original_ptr == NULL)
        return NULL;

    // 计算对齐后的地址
    void *aligned_ptr = (void*)(((size_t)original_ptr + alignment - 1 + sizeof(void*)) & ~(alignment - 1));
...
static void spng_aligned_free(void *aligned_ptr)
{
    if (aligned_ptr)
        tkl_system_psram_free(((void**)aligned_ptr)[-1]);
}
#endif

static void *my_spng_malloc(size_t size)
{
#ifdef SPNG_MEMORY_ALIGNED
    return spng_aligned_malloc(SPNG_MEMORY_ALIGNED_SIZE, size);           //4字节对齐
#else
    return tkl_system_psram_malloc(size);
#endif
}

static void *my_spng_realloc(void *ptr, size_t size)
{
#ifdef SPNG_MEMORY_ALIGNED
    void *tmp;

    tmp = spng_aligned_malloc(SPNG_MEMORY_ALIGNED_SIZE, size);
    if (tmp && ptr) {
        memcpy(tmp, ptr, size);
        #if 0
        if (size & 0x3) {
...
static void *my_spng_calloc(size_t num, size_t size) {
#ifdef SPNG_MEMORY_ALIGNED
    void *ptr = spng_aligned_malloc(SPNG_MEMORY_ALIGNED_SIZE, num * size);
#else
    void *ptr = tkl_system_psram_malloc(num * size);
#endif
    if (ptr == NULL) {
        return NULL;
    }
...
static void my_spng_free(void *ptr)
{
#ifdef SPNG_MEMORY_ALIGNED
    spng_aligned_free(ptr);
#else
...
spng_ctx *spng_ctx_new(int flags)
{
    struct spng_alloc alloc =
    {
        .malloc_fn = my_spng_malloc,
        .realloc_fn = my_spng_realloc,
        .calloc_fn = my_spng_calloc,
        .free_fn = my_spng_free
    };
...
#ifdef SIMD_USE_CMSIS_DSP
#ifdef SIMD_USE_CMSIS_DSP_DEFILTER_ACCELERATE
#include "arm_math.h"

static void defilter_sub3(size_t rowbytes, unsigned char *row) {
    q31_t *rp = (q31_t*)row; // 强制对齐到4字节
    const q31_t *rp_end = (q31_t*)(row + rowbytes);
    q31_t prev = 0;

    assert((uintptr_t)row % 4 == 0); // 确保对齐

    while (rp < rp_end) {
        q31_t curr = *rp;
        // 4字节并行加法: prev + curr
        q31_t result = __UADD8(prev, curr);
        *rp++ = result;
        prev = result; // 传递累加结果
    }

    // 处理末尾不足4字节的数据
    size_t remain = rowbytes % 4;
    if (remain > 0) {
        uint8_t *tail = row + (rowbytes - remain);
        for (size_t i = 0; i < remain; i++) {
            tail[i] += (prev >> (24 - i * 8)) & 0xFF; // 按字节处理
        }
    }
}
...
static void defilter_sub4(size_t rowbytes, unsigned char *row) {
    ...
    for (; rp < rp_end; rp += 4) { // 每次处理16字节（4个q31_t）
        q31_t d0 = rp[0], d1 = rp[1], d2 = rp[2], d3 = rp[3];
        // 链式4字节并行加法
        d0 = __UADD8(prev, d0);
        d1 = __UADD8(d0, d1);
        d2 = __UADD8(d1, d2);
        d3 = __UADD8(d2, d3);
        rp[0] = d0; rp[1] = d1; rp[2] = d2; rp[3] = d3;
        prev = d3;
    }
}
...
static void defilter_avg3(size_t rowbytes, unsigned char *row, const unsigned char *prev_row) {
    ...
    while (rp < rp_end) {
        ...
        q31_t p = *pp++;
        // 计算平均值: (prev + p) / 2 + curr
        q31_t avg = __UHADD8(prev, p); // 硬件半加
        q31_t result = __UADD8(avg, curr);
        *rp++ = result;
        prev = result;
    }
}
...
static void defilter_avg4(size_t rowbytes, unsigned char *row, const unsigned char *prev_row) {
    ...
    assert((uintptr_t)row % 4 == 0 && (uintptr_t)prev_row % 4 == 0);

    for (; rp < rp_end; rp += 4, pp += 4) { // 每次处理16字节（4个q31_t）
        q31_t d0 = rp[0], d1 = rp[1], d2 = rp[2], d3 = rp[3];
        q31_t p0 = pp[0], p1 = pp[1], p2 = pp[2], p3 = pp[3];

        // 链式平均计算：avg(prev, pX) + dX
        d0 = __UADD8(__UHADD8(prev, p0), d0);
        d1 = __UADD8(__UHADD8(d0,    p1), d1);
        d2 = __UADD8(__UHADD8(d1,    p2), d2);
        d3 = __UADD8(__UHADD8(d2,    p3), d3);
...
static q31_t paeth_m4(q31_t a, q31_t b, q31_t c) {
    // 计算 p = a + b - c
    q31_t p = __USUB8(__UADD8(a, b), c);
    
    // 计算 |a - c| 和 |b - c|
    q31_t ac_diff = __USUB8(a, c);
    q31_t ac_neg = __USUB8(c, a);
    q31_t pa = __SEL(ac_diff, ac_neg); // 绝对值
    
    q31_t bc_diff = __USUB8(b, c);
    q31_t bc_neg = __USUB8(c, b);
    q31_t pb = __SEL(bc_diff, bc_neg);
    
    // 计算 |p - c|
    q31_t pc_diff = __USUB8(p, c);
    q31_t pc_neg = __USUB8(c, p);
    q31_t pc = __SEL(pc_diff, pc_neg);
    
    // 逐字节生成掩码
    q31_t mask_a = (pa <= pb) & (pa <= pc);
    q31_t mask_b = (pb <= pc) & ~mask_a;
    q31_t mask_c = ~(mask_a | mask_b);
    
    return (mask_a & a) | (mask_b & b) | (mask_c & c);
}
...
static void defilter_paeth3(size_t rowbytes, unsigned char *row, const unsigned char *prev_row) {
    ...
    const q31_t *pp = (q31_t*)prev_row;
    const q31_t *rp_end = (q31_t*)(row + rowbytes);
    q31_t prev = 0, prev_upper = 0;

    while (rp < rp_end) {
        q31_t curr = *rp;
        q31_t upper = *pp++;
        q31_t pred = paeth_m4(prev, upper, prev_upper);
        *rp++ = __UADD8(pred, curr);
        prev_upper = upper;
        prev = curr;
    }
}
...
static void defilter_paeth4(size_t rowbytes, unsigned char *row, const unsigned char *prev_row) {
    ...
    for (; rp < rp_end; rp += 4, pp += 4) { // 每次处理16字节
        q31_t d0 = rp[0], d1 = rp[1], d2 = rp[2], d3 = rp[3];
        q31_t p0 = pp[0], p1 = pp[1], p2 = pp[2], p3 = pp[3];

        // Paeth 预测链式计算
        d0 = __UADD8(paeth_m4(prev, p0, prev_upper), d0);
        d1 = __UADD8(paeth_m4(d0,  p1, p0),          d1);
        d2 = __UADD8(paeth_m4(d1,  p2, p1),          d2);
        d3 = __UADD8(paeth_m4(d2,  p3, p2),          d3);

        rp[0] = d0; rp[1] = d1; rp[2] = d2; rp[3] = d3;
        prev_upper = p3; // 更新上方的历史值
        prev = d3;       // 更新左侧的历史值
    }
}
#endif

static uint32_t expand_palette_rgba8_neon(uint8_t *row, const uint8_t *scanline, const uint8_t *plte, uint32_t width)
{
    const uint32_t *palette = (const uint32_t*)plte;
    for (uint32_t i = 0; i < width; i++)
    {
        uint32_t index = scanline[i];
        *((uint32_t*)row) = palette[index];
        row += 4;
    }
    return width * 4;
}

static uint32_t expand_palette_rgb8_neon(uint8_t *row, const uint8_t *scanline, const uint8_t *plte, uint32_t width)
{
    for (uint32_t i = 0; i < width; i++)
    {
        uint32_t index = scanline[i];
        row[0] = plte[3 * index];
        row[1] = plte[3 * index + 1];
        row[2] = plte[3 * index + 2];
        row += 3;
    }
    return width * 3;
}
