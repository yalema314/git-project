...
#define SWAP_( x, y )  do                \
  ...
  Render_Glyph( RAS_ARG )
  {
    ...
    if (buffer == NULL) {
        buffer = (Long *)ft_smalloc(FT_MAX_BLACK_POOL * sizeof(Long));
        if (buffer == NULL) {
            FT_TRACE6(( "Render Glyph: malloc fail\n" ));
            return FT_THROW( Out_Of_Memory );
        }
    }
    FT_MEM_ZERO(buffer, FT_MAX_BLACK_POOL * sizeof(Long));
    ras.buff     = buffer;
    //ras.sizeBuff = (&buffer)[1]; /* Points to right after buffer. */
    ras.sizeBuff = buffer + FT_MAX_BLACK_POOL; /* 指向缓冲区末尾. */
