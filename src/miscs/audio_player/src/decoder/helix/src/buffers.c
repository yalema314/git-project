...
#else
extern void *tal_malloc(int size);
extern void tal_free(void *ptr);
static void *(*helix_malloc)(int size) = tal_malloc;
static void (*helix_free)(void *buff) = tal_free;
void MP3SetBufferFunc(void *(*malloc)(int size), void (*free)(void *buff))
{
	helix_malloc = malloc;
	helix_free = free;
}
...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)helix_malloc(sizeof(MP3DecInfo));
