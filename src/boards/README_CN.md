...
> 注意
>
> 当前 `SuperT` 默认构建并不是同时启用了多块硬件板。当前实际生效的硬件基板是 `T5AI_BOARD`，其余 `EVB/EYES/ROBOT/DESKTOP` 板型宏默认都关闭。
>
> 当前默认构建之所以看起来“带了 EYES”，是因为 `T5AI_BOARD` 在当前工程里默认开启了 `CONFIG_SUPERT_USE_EYES_UI`，因此会复用 `T5AI_BOARD_EYES` 的 UI 素材和界面入口，但 GPIO、显示引脚和板级初始化仍来自 `T5AI_BOARD`。
>
> 当前工程实际生效配置请优先参考 [当前板载配置](../../docs/当前板载配置.md) 和 [OTA升级说明](../../docs/OTA升级说明.md)。
